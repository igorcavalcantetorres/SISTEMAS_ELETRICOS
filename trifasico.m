...
theta = abs(Van_ang-Ia_ang);
P3f = (sqrt(3)*Vab_mod*Ia_mod*cosd(theta))/1000;
Q3f = (sqrt(3)*Vab_mod*Ia_mod*sind(theta))/1000;
FP_t = abs(P3f/S3f);
...
alpha_exp = [1 -0.5-sqrt(3)/2i -0.5+sqrt(3)/2i]; %Matriz de sequência negativa
V_f_seq = V_f*alpha_exp; %Composição da matriz de tensões de fase sequencial
cte = 1.5+0.866025404i; %relação para a tensão de linha
%Cálculo dos módulos e ângulos das tensões
Van_mod = abs(V_f_seq(1));
